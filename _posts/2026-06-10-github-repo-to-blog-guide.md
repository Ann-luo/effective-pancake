...
> **难度**：纯小白友好  
> **费用**：免费  
> **效果预览**：[ann-luo.github.io/effective-pancake](https://ann-luo.github.io/effective-pancake/)
...
以 [Ann-luo/effective-pancake](https://github.com/Ann-luo/effective-pancake) 为例，搭建完博客后整个仓库长这样：

> 📌 所有文件名都可以点击，直接跳转到 GitHub 查看源码。
...
├── .github/
│   └── workflows/
│       └── jekyll.yml              ← 自动部署脚本（核心）
│
├── _posts/                         ← 📝 博客文章存放处（核心）
│   ├── 2026-06-08-vscode-copilot-proxy-fix.md
│   ├── 2026-06-08-vscode-copilot-proxy-chatlog.md
│   ├── 2026-06-08-windows-symlink-guide.md
...
| 文件/目录 | 作用 | 路径 |
|-----------|------|------|
| `_config.yml` | 博客配置：标题、主题、URL 等 | [_config.yml](https://github.com/Ann-luo/effective-pancake/blob/main/_config.yml) |
| `index.md` | 博客首页，自动展示文章列表 | [index.md](https://github.com/Ann-luo/effective-pancake/blob/main/index.md) |
...
> 这一步只需要做一次。之后GitHub Actions会自动处理部署。
...
- 没创建 Gemfile → 创建一个
- 没有 `gem "minima"` → 加上去
- 用了 `bundler-cache: true` 但没有 `Gemfile.lock` → 改成先 `bundle install`

### 能改主题吗？

能。GitHub 内置支持的主题都在 [pages.github.com/themes](https://pages.github.com/themes/) 能看到。比如把 `_config.yml` 里的 `theme: minima` 改成 `theme: cayman` 就换了皮肤。
