---
name: codex-chat
description: 通过 CDP (Chrome DevTools Protocol) 与 Codex (OpenAI 桌面客户端) 通信——发消息、读回复、多轮对话
---
...
- 辅助脚本位于: `~/.claude/skills/scripts/codex_cdp_helper.js`
...
```bash
node ~/.claude/skills/scripts/codex_cdp_helper.js send "你的消息内容"
```
...
```bash
node ~/.claude/skills/scripts/codex_cdp_helper.js read
```
...
以下任意说法都可以触发此 skill：
- "和 codex 聊天" / "跟 codex 对话"
- "给 codex 发消息" / "发消息给 codex"
- "看看 codex 回了什么" / "codex 回复了吗"
- "回复 codex" / "回 codex"
- `/codex-chat`
